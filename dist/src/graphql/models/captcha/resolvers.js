...
const addCaptcha = (root, args, context, schema) => __awaiter(void 0, void 0, void 0, function* () {
    const { user, ip, role } = context;
    // 参数声明
    let err, result, fields;
    [err, fields] = tools_1.getSave({ args, model: Model.addCaptcha, role });
    if (err)
        throw errors_1.default({ message: err });
    ...
    if (type == 'email-unlock-token') {
        if (!user)
            throw errors_1.default({ message: '未登陆' });
    ...
    if (type == 'forgot' && phone) {
        [err, result] = yield to_1.default(models_1.Phone.findOne({ query: { phone } }));
        if (err)
            throw errors_1.default({ message: err });
        if (!result)
            throw errors_1.default({ message: '手机号码不存在' });
        phone = result.phone;
        area_code = result.area_code;
    }
    // =========================
    ...
    if (type == 'forgot' && email) {
        [err, result] = yield to_1.default(models_1.Account.findOne({ query: { email } }));
        if (err)
            throw errors_1.default({ message: err });
        if (!result)
    ...
    if (!email && !phone && !area_code) {
        if (type != 'sign-in') {
            throw errors_1.default({ message: 'type 错误' });
        }
        ...
        if (!result)
            return { success: true, _id: '', url: '' };
        [err, result] = yield to_1.default(models_1.Captcha.create({ ip, type }));
        return { success: true, _id: result._id, url: domain + '/captcha/' + result._id };
    }
    // =========================
    // 发送验证码到邮箱
    if (email && type) {
        [err, result] = yield to_1.default(sendEmail({ user, email, type }));
        if (err)
            throw errors_1.default({ message: err });
    }
    // =========================
    ...
    if (phone && area_code && type) {
        [err, result] = yield to_1.default(sendSMS({ user, area_code, phone, type }));
        if (err)
            throw errors_1.default({ message: err });
    }
    return {
...
exports.query = { getCaptcha };
exports.mutation = { addCaptcha };
// 发送验证码到邮箱
const sendEmail = ({ user, email, type }) => {
    return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        let [err, res] = yield to_1.default(models_1.Account.findOne({
            query: { email },
            options: {
                populate: [{
                        path: 'user_id',
        ...
        if (type == 'binding-email') {
            if (!user)
                return reject('无权访问');
            if (res)
                return reject('邮箱已经被注册');
            data.user_id = user._id;
            [err, res] = yield to_1.default(models_1.Captcha.create(data));
            ...
            if (user)
                return reject('登陆用户不能注册，请退出账户后注册');
            if (res)
                return reject('邮箱已被注册');
            [err, res] = yield to_1.default(models_1.Captcha.create(data));
        ...
        [err] = yield to_1.default(Email.send(mailOptions));
        if (err) {
            reject(res);
        }
        else {
...
const sendSMS = ({ user, area_code = '', phone, type }) => {
    return new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        [err, phoneAccount] = yield to_1.default(models_1.Phone.findOne({
            query: { phone }
        }));
        // 判断区号是否有效
        ...
        if (type == 'binding-phone' || type == 'reset-phone') {
            ...
            if (!existAreaCode)
                return reject('区号不存在');
            if (phoneAccount)
                return reject('手机号已被注册');
            data.user_id = user._id;
            data.area_code = area_code;
        }
        else if (type == 'forgot') {
            if (!phoneAccount)
                return reject('手机号不存在');
        }
        else if (type == 'sign-up') {
            if (!existAreaCode)
                return reject('区号不存在');
        ...
        [err] = yield to_1.default(serviceProvider.sendSMS({
